...
export async function GET(_request: Request, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (path.length < 4 || path.length > 30) return NextResponse.json({ error: "Invalid provision path" }, { status: 400 });
  try {
    const data = await getSectionPageData(path);
    if (!data) return NextResponse.json({ error: "Source provision unavailable" }, { status: 404 });
    ...
    for (const provision of data.provisions) {
      blocks.push({ anchor: provision.anchor, heading: provision.rule.heading || provision.designator, body: provision.rule.body ?? "", citationPath: provision.rule.citation_path ?? data.citationPath, refs: [] });
    }
    const source: WorkspaceSource = { citationPath: data.citationPath, heading: data.root.heading ?? null, officialUrl: data.root.source_url ?? null, effectiveDate: data.root.effective_date ?? null, focusAnchor: data.focusAnchor, truncated: data.truncated, blocks };
